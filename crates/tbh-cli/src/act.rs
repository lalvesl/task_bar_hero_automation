...
//! being picked, without a task having to exist first. The tasks in M6 and M7
//! call the same crate; nothing here is a parallel implementation.
...
pub fn click(target: &Target, args: &ClickArgs) -> anyhow::Result<()> {
    ...
    let (screen_x, screen_y) = at.resolve(rect).context("resolving the click point")?;
    input.click(at, rect, button).context("clicking")?;
...
pub fn key(target: &Target, args: &KeyArgs) -> anyhow::Result<()> {
    // The window is located first so a key never lands on a display where the
    // game is not actually running.
    let capture = connect(target)?;
    drop(capture);
...
fn connect(target: &Target) -> anyhow::Result<X11Capture> {
    let capture = X11Capture::connect(&target.display, &target.window).with_context(|| {
        format!(
